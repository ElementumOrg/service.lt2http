...
from lt2http.daemon import shutdown
from lt2http.addon import ADDON, ADDON_ID, ADDON_PATH
from lt2http.osarch import PLATFORM
from lt2http.unicode import to_unicode
from lt2http.util import notify, getLocalizedString, getLocalizedLabel, getLocalizedText
...
def getInfoLabels():
    ...
    if not tmdb_id:
        ...
        if 'tmdb' in query and 'type' in query and query['type'][0] == 'search':
            ...
            url = "%s/search/infolabels/%s?index=%s" % (LT2HTTP_HOST, tmdb_id, query['index'])
            ...
            tmdb_id = query['tmdb'][0]
            url = "%s/movie/%s/infolabels" % (LT2HTTP_HOST, tmdb_id)
        elif 'show' in query:
            tmdb_id = query['show'][0]
            if 'season' in query and 'episode' in query:
                url = "%s/show/%s/season/%s/episode/%s/infolabels" % (LT2HTTP_HOST, tmdb_id, query['season'][0], query['episode'][0])
            else:
                url = "%s/show/%s/infolabels" % (LT2HTTP_HOST, tmdb_id)
        else:
            url = "%s/infolabels" % (LT2HTTP_HOST)
    elif 'movie' in sys.argv[0]:
        url = "%s/movie/%s/infolabels" % (LT2HTTP_HOST, tmdb_id)
    elif ('episode' in sys.argv[0] or 'show' in sys.argv[0]) and len(id_list) > 2:
        url = "%s/show/%s/season/%s/episode/%s/infolabels" % (LT2HTTP_HOST, tmdb_id, id_list[1], id_list[2])
    elif 'show' in sys.argv[0] and len(id_list) == 2:
        url = "%s/show/%s/season/%s/episode/%s/infolabels" % (LT2HTTP_HOST, tmdb_id, id_list[1], 1)
    else:
        url = "%s/infolabels" % (LT2HTTP_HOST)
...
def _json(url):
    with closing(urllib_request.urlopen(url)) as response:
        if response.code == 300:
            ...
            # Pause currently playing lt2http file to avoid doubling requests
            try:
...
def run(url_suffix="", retry=0):
    ...
    url = sys.argv[0].replace("plugin://%s" % ADDON_ID, LT2HTTP_HOST + url_suffix) + sys.argv[2]
    query_add = ""
