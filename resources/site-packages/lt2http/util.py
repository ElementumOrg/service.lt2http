...
def download_github_folder(repo, folder, destination):
    ...
    log.debug("Downloading repo content for folder: %s, to folder: %s" % (contents_url, destination))
    try:
        with requests.get(contents_url) as r:
            ...
            for i in lists:
                ...
                dest = os.path.join(destination, i['download_url'].rsplit('/', 1)[1])
                log.debug("Downloading file '%s' to '%s'" % (i['download_url'], dest))
